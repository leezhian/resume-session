...
The wrapper sets `TOOL` to `claude`, `codex`, `cursor`, `qoder`, or `grok` and sets
...
```text
session_reader.py <claude|codex|cursor|qoder|grok> <list|show> [ref] [--cwd DIR] [--within-min N] [--json] [--max-tool-chars N]
```
